...
import { useNavigate } from 'react-router';
    ...
    useEffect(() => {
        ...
        return () => {
            document.head.removeChild(style); // bersihin waktu komponen di-unmount
        };
    }, []);
        ...
        const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
            event.preventDefault()
                ...
                if(!formData.country || !formData.travelStyle || !formData.interest || !formData.budget || !formData.groupType)  {
                    setError('Isi semua kolomnya !');
                    setloading(false);
                    return;
                }

                if(formData.duration < 1 || formData.duration > 10 ){
                    setError('Durasi harus berkisar diantara 1 sampai 10 hari');
                    setloading(false);
                    return;
                }
    ...
    return (
        <main className='flex flex-col gap-10 pb-20 wrapper'>
            ...
            <section className='mt-2.5 wrapper-empty'>
                <form className='trip-form' onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <input type='number' id='duration' name='duration' placeholder='Enter a number of days (5, 12 ....)' className='form-input placeholder:text-gray-100' onChange={(e) => handleChange('duration', Number(e.target.value))}/>
                    </div>
